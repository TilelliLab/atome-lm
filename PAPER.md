...
High entropy means the router could not decide which compute primitive was most appropriate for the position. In our larger V2 production model (3 M parameters, mixed-domain training) router entropy is observed to track out-of-domain inputs and to correlate with per-token loss; the signal is structural — it requires no uncertainty-specific training and no extra parameters. At the Atome-LLM engine-default scale (60 K parameters, single narrow corpus) the signal is exposed identically, but its calibration as an uncertainty estimator at this scale is not proven here. Measuring it is a separate exercise (see e.g. expected calibration error between router entropy and per-token loss).
